...
run_it <- function(data_out = FALSE, miss = FALSE, check_flname = FALSE, chkskp = 4, chkcol = 2, analspecfile = '/SettingsFiles/AnalysisSpec.csv'){
  ...
  settings <- get_setns(analspecfile) # Get settings
      ...
      if (file.access(file_i, mode = 4) == 0) { # If can read from file
        ...
        for (i in 1:sum(settings$unfill, settings$interp) ) {
          ...
          res_l <- c( # Call calculate functions and put all items together in a single list
              ...
              if (settings$fgap) {
                c(FrameGaps = "", gaps_l) # append frame gap counts if present
              } else {
                ""
              }
            )
    ...
    res_df[ , c(-1:-3, -7, -17)] <- apply(res_df[ , c(-1:-3, -7, -17)], 2, as.numeric)
    ...
    if (miss) {
      if (settings$interp & settings$fgap) {
        ...
        res_df %>% # Calculate cummulative gap counts
          filter(., !Interp) %>% 
          select(., matches("\\d{4}") ) %>% 
          apply(., 2, sum) %>% # Sum each column
          cumsum() %>% # Cumulative sum of
          data.frame(CumulativeCount = .) %>% # Convert to df
          cbind(GapSize = as.integer(rownames(.)), .) %>% 
          assign("gap_counts",.[-1] ,envir = .GlobalEnv) # The cumulative count of each number of frame gaps for all data sets
        
        res_df %>% # Calculate the maximum size of missing frames
          colnames(.) %>% # Get column names...
          .[length(.)] %>% # Take last item...
